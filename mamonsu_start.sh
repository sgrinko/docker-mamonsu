...
sed -i "s/host = PGHOST/host = ${PGHOST:-127.0.0.1}/g" ${CONFIG_DIR}/agent.conf \
      ...
      && sed -i "s/host = MAMONSU_AGENTHOST/host = $MAMONSU_AGENTHOST/g" ${CONFIG_DIR}/agent.conf \
...
TABLE_CHECK="mamonsu_timestamp_master_${TABLE_CHECK/'.'/'_'}"
if psql -qtAX -c "select case when not pg_is_in_recovery() and not exists(select * from pg_class where relname = '$TABLE_CHECK') then 1 else 0 end as master" mamonsu | grep '1' ; then
   ...
   if [ "$PGPASSWORD" = "" ]; then
     /usr/bin/mamonsu bootstrap -M mamonsu -U postgres -d mamonsu --port=${PGPORT:-5432} --host=${PGHOST:-127.0.0.1};
   else
     /usr/bin/mamonsu bootstrap -M mamonsu -U postgres -d mamonsu --port=${PGPORT:-5432} --password=$PGPASSWORD --host=${PGHOST:-127.0.0.1};
   fi
fi
